...
static pkey_set_t real_pkey_set(void) {
    ...
    if (!fn) {
        ...
        /* dlsym can return NULL if symbol missing; that's fine, we'll treat as deny if missing */
    }
    return fn;
}

/* helper: get resolved filename owning a return address */
...
/* Compare allowed module list: ALLOW_PKEY_MODULE can be set to
   ...
   You may also allow comma-separated list. */
static int caller_allowed(void *retaddr) {
    const char *allow = getenv("ALLOW_PKEY_MODULE");
    if (!allow || !allow[0]) return 0; // no allowed modules => deny by default
    ...
    while (token) {
        ...
        if (strcmp(owner, token) == 0) { free(copy); return 1; }
        // compare basename
        const char *b = strrchr(owner, '/');
        const char *ownerbase = b ? b+1 : owner;
        if (strcmp(ownerbase, token) == 0) { free(copy); return 1; }
        // allow token that is a substring (looser)
        if (strstr(owner, token) != NULL) { free(copy); return 1; }
...
int pkey_set(int pkey, unsigned int rights) {
    ...
    if (caller_allowed(ret)) {
        ...
        if (!fn) {
            // real symbol not present: deny or emulate
            errno = ENOSYS;
            return -1;
        }
